...
- **Research Objectives — verified in full on 2026-08-26 after a teammate (Jared) flagged that the objectives looked changed.** Every line changed: the Main Objective, all three Specific Objectives, and all six Expected Outputs. Full before/after below, plus what was checked to confirm this isn't scope creep.

  **Main Objective**
  **BEFORE (defense manuscript, June 2024):** "To investigate the performance of a developed Multi-Agent Reinforcement Learning (MARL) scheduling model for dynamic bus scheduling under non-ideal traffic conditions on a calibrated simulation of the EDSA Carousel corridor."
  **AFTER (live manuscript):** "To evaluate a parameter-sharing Multi-Agent Reinforcement Learning scheduling model under baseline and disturbed operating conditions in a public-data-calibrated simulation of CapMetro Rapid Route 801 direction code 6."

  **Specific Objective 1 (Environment)**
  **BEFORE:** "Environment Construction: To construct a stochastic traffic simulation environment representing a defined traffic area." — EO 1.1: "A simulation model of a defined operational scope, validated against empirical traffic data..." — EO 1.2: "An environment simulator capable of introducing environmental variability through adjustable parameters..."
  **AFTER:** "Environment construction: construct a reproducible Route 801 simulation from the cleaned CapMetro APC subset and temporally aligned NOAA weather observations." — EO 1.1 (new deliverable): "A documented acquisition and preprocessing pipeline with source URLs, queries, cleaning rules, row counts, checksums, field mappings, time-zone handling, and unresolved data gates." — EO 1.2: "A corridor model validated against held-out empirical stop-event counts and segment travel times..." — EO 1.3 (new): "Configurable demand-surge, observed-weather, synthetic severe-weather, and synthetic breakdown components whose activation is explicit in each experiment."

  **Specific Objective 2 (Algorithm)**
  **BEFORE:** "Algorithm Implementation: To develop and implement a Multi-Agent Reinforcement Learning (MARL) approach for dynamic bus scheduling." — EO 2.1: "A formulated agent architecture defining individual bus units as agents with discrete state and action space with a continuous reward function that punishes irregularities and passenger service delays."
  **AFTER:** "Algorithm implementation: develop a shared-parameter DDQN bus-control policy with per-agent local observations, per-agent rewards, a shared replay buffer, and decentralized execution." — EO 2.1: "A fully specified observation, action, reward, transition, and training protocol, including reward-weight sensitivity analysis and reproducible random-seed control."

  **Specific Objective 3 (Evaluation)**
  **BEFORE:** "Performance Evaluation Under Ideal and Non-Ideal Conditions: To evaluate the scheduling performance of the MARL approach under both ideal operating conditions and non-ideal conditions." — EO 3.1/3.2: generic "quantitative and statistical assessment... measuring passenger waiting time, travel time, and headway coefficient of variation," reported under ideal / degradation-under-non-ideal.
  **AFTER:** "Performance evaluation: compare MARL with No Control, Forward Headway, and Even Headway controllers under a declared baseline, combined-disturbance test, and single-disturbance ablations." — EO 3.1: "Matched-seed estimates and uncertainty intervals for simulated passenger waiting time, bus travel time, and headway coefficient of variation under the baseline evaluation." — EO 3.2: "Matched-seed robustness estimates under combined and isolated demand, weather, and breakdown stress, reported relative to the baseline without labeling synthetic intensities as observed meteorological categories."

  **Verification performed (2026-08-26):** confirmed the underlying study structure is unchanged — both versions have the same three-part objective shape (build the environment, implement the algorithm, evaluate performance) and the same three response metrics (passenger waiting time, travel time, headway coefficient of variation). Two things could have looked like scope creep and were checked against the defense's own Chapter 3:
  - **DDQN, parameter sharing/CTDE, and the No Control/Forward Headway/Even Headway baselines were already in the defense manuscript's methods.tex** (14, 6, and 6 mentions respectively) — the live objectives are catching up to name what Chapter 3 already specified, not introducing a new algorithm or new baselines.
  - **"Reward-weight sensitivity analysis" and "random-seed control" were also already in the defense's methods.tex** (Expected Output 2.1 and a "Random Seed Control" subsection both existed at defense time) — same conclusion, wording catch-up not new scope.
  - The one genuinely **new** deliverable is EO 1.1's "documented acquisition and preprocessing pipeline" — this did not exist in the defense version in any form. It directly answers RTC panel comments E1C1/E2C5/E4C22 ("describe the dataset," "explain what it looks like," "describe dataset contents explicitly"), so it is a deliberate, justified addition responding to a real panel requirement, not an arbitrary change.
  - **Conclusion for Jared:** the objectives did change — every sentence was reworded — but the change is (a) the expected consequence of swapping the empirical basis from EDSA to CapMetro Route 801 (same pattern as every other section of problem.tex), and (b) in the two places where it looks like new scope (DDQN specifics, sensitivity analysis / seed control), that content already existed in the defense's own Chapter 3 — the objectives were simply out of sync with the methods chapter until this rewrite. The one truly new item (EO 1.1) is a panel-mandated addition.
...
"This study addresses the obstacle by adopting the lognormal travel-time parameterization that Patil et al. validated against **INRIX freeway data** via the Kolmogorov-Smirnov test, and then sweeping the disturbance intensity across a range of validated and extrapolated CV values rather than fitting a single point estimate."

**AFTER**

"This study addresses the obstacle by adopting the lognormal travel-time parameterization that Patil et al. validated **for an INRIX-anchored urban-arterial corridor** via the Kolmogorov-Smirnov test, and then sweeping the disturbance intensity across a range of validated and extrapolated CV values rather than fitting a single point estimate."

**Why:** the 2026-08-06 "Citation fix: Patil2025Conformal" entry established (against the actual paper, whose Table V classifies the route as "Local, Minor/Principal Arterials") that Patil et al.'s corridor is an urban arterial, NOT a freeway — and that the KS test checks the simulated distribution's shape rather than being a direct comparison against INRIX data. That fix only corrected methods.tex; this introduction.tex sentence still said "INRIX freeway data," contradicting both the corrected methods.tex and another sentence in the same introduction ("Patil et al. validate a lognormal travel-time form for simulated arterial scenarios"). The error predates the Texas pivot; it was found during the 2026-08-25 full read-through. This edit completes the original citation fix and removes the internal contradiction.
